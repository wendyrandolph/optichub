...
@section('title', 'New Service Location')

@section('trades-content')
    @php
        $selectedClientId = old('client_id', $selectedClient?->id);
        $selectedCompanyId = old('client_company_id', $selectedClient?->client_company_id);
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div class="flex flex-col gap-3">
            ...
            <div>
                ...
                <h1 class="text-2xl font-semibold text-text-base">New Service Location</h1>
                <p class="text-sm text-text-subtle mt-1">Add a location where this client receives service.</p>
            </div>
        </div>
        ...
        <div class="oh-card border border-border-default/60 rounded-2xl p-4 md:p-6">
            <form method="POST" action="{{ route('tenant.trades.locations.store', ['tenant' => $tenantKey]) }}"
                class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input id="label" name="label" class="oh-input h-10" value="{{ old('label') }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="city" name="city" class="oh-input h-10" value="{{ old('city') }}">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-sm font-medium text-text-base" for="state">State/Region</label>
                        <input id="state" name="state" class="oh-input h-10" value="{{ old('state') }}">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-sm font-medium text-text-base" for="postal_code">Postal code</label>
                        <input id="postal_code" name="postal_code" class="oh-input h-10" value="{{ old('postal_code') }}">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-sm font-medium text-text-base" for="country">Country</label>
                        <input id="country" name="country" class="oh-input h-10" value="{{ old('country') }}">
                    </div>

                    <div class="space-y-1.5 md:col-span-2">
                        <label class="text-sm font-medium text-text-base" for="access_notes">Access notes</label>
                        <textarea id="access_notes" name="access_notes" class="oh-input min-h-[110px]" rows="3">{{ old('access_notes') }}</textarea>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-border-default/60">
                    ...
                    <button class="oh-btn oh-btn--primary" type="submit">Create location</button>
                </div>
            </form>
        </div>
    </div>
