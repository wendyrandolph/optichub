...
@section('trades-content')
    @php
        ...
        $itemRows = old('items', $job->items->map(fn($item) => [
            'description' => $item->description,
            'quantity' => $item->quantity,
            'unit_price' => $item->unit_price,
            'warranty_ends_on' => optional($item->warranty_ends_on)->toDateString(),
            'warranty_terms' => $item->warranty_terms,
        ])->toArray());
        if (!is_array($itemRows)) {
            $itemRows = [];
        }
        $checklistRows = old('checklist', $job->checklistItems->map(fn($item) => [
            'label' => $item->label,
        ...
        $prefillPropertyType = old('property_type', $job->property_type ?? ($workType === 'both' ? '' : $workType));
    @endphp
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div class="flex flex-col gap-3">
            ...
            <div>
                ...
                <h1 class="text-2xl font-semibold text-text-base">Edit Trade Job</h1>
                <p class="text-sm text-text-subtle mt-1">Update client details, schedule notes, and job scope.</p>
            </div>
        </div>
        ...
        <div class="oh-card border border-border-default/60 rounded-2xl p-4 md:p-6">
            <form method="POST" action="{{ route('tenant.trades.jobs.update', ['tenant' => $tenantKey, 'job' => $job->id]) }}"
                class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input id="summary" name="summary" class="oh-input h-10" required
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="type" name="type" class="oh-select h-10">
                            <option value="service" @selected(old('type', $job->type) === 'service')>Service job</option>
                            <option value="project" @selected(old('type', $job->type) === 'project')>Project job</option>
                        </select>
                    </div>
                            ...
                            @foreach ($clients as $client)
                                @php
                                    $clientLabel =
                                        ...
                                        'Client #' . $client->id;
                                @endphp
                                <option value="{{ $client->id }}" @selected((string) old('client_id', $job->client_id) === (string) $client->id)>
                                    {{ $clientLabel }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <select id="service_location_id" name="service_location_id" class="oh-select h-10">
                            ...
                            @foreach ($locations as $location)
                                ...
                                @endphp
                                <option value="{{ $location->id }}" @selected((string) old('service_location_id', $job->service_location_id) === (string) $location->id)>
                        ...
                        @if ($locations->isEmpty())
                            <p class="text-xs text-text-subtle">No service locations for this client yet.</p>
                        @endif
                    </div>

                    <div class="space-y-1.5 md:col-span-2">
                        <label class="text-sm font-medium text-text-base" for="description">Description</label>
                        <textarea id="description" name="description" class="oh-input min-h-[120px]" rows="4">{{ old('description', $job->description) }}</textarea>
                    </div>
                    ...
                    @if ($warrantyScope === 'job')
                        <div class="space-y-1.5">
                            ...
                            <input id="warranty_ends_on" name="warranty_ends_on" type="date" class="oh-input h-10"
                                value="{{ old('warranty_ends_on', optional($job->warranty_ends_on)->toDateString()) }}">
                        </div>
                        <div class="space-y-1.5 md:col-span-2">
                            ...
                            <input id="warranty_terms" name="warranty_terms" class="oh-input h-10"
                                value="{{ old('warranty_terms', $job->warranty_terms) }}" placeholder="e.g. Labor covered for 12 months">
                        </div>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-border-default/60">
                    <a href="{{ route('tenant.trades.jobs.show', ['tenant' => $tenantKey, 'job' => $job->id]) }}" class="oh-btn">Cancel</a>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (checklistBody && checklistTemplate && addChecklist) {
                    const addRow = () => {
                        const index = checklistBody.children.length;
