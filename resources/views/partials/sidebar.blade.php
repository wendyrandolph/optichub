@php
    ...
    $isPlatformOwner = $currentUser?->isPlatformOwner() ?? false;
...
$isProviderAdmin = $isAdminGuard && in_array($role, ['provider', 'admin', 'super_admin', 'superadmin'], true);
...
$pinnedOptions = [
    ...
    'tenant.calendar.index' => ['icon' => 'fa-calendar-days', 'label' => 'Calendar', 'match' => 'tenant.calendar.*'],
    'tenant.time.index' => ['icon' => 'fa-clock', 'label' => 'Time', 'match' => 'tenant.time.*'],
    'tenant.invoices.index' => ['icon' => 'fa-file-invoice', 'label' => 'Client Invoices', 'match' => 'tenant.invoices.*'],
    'tenant.contacts.index' => ['icon' => 'fa-user-group', 'label' => 'Clients', 'match' => 'tenant.contacts.*'],
    'tenant.companies.index' => ['icon' => 'fa-building', 'label' => 'Client Companies', 'match' => 'tenant.companies.*'],
    'tenant.opportunities.index' => ['icon' => 'fa-chart-line', 'label' => 'Opportunities', 'match' => 'tenant.opportunities.*'],
    ...
    'tenant.dashboards.index' => ['icon' => 'fa-gauge-high', 'label' => 'Dashboard', 'match' => 'tenant.dashboards.*'],
];
...
$pinnedRoutes = array_values(
    array_unique(array_filter($pinnedRoutes, fn($r) => isset($pinnedOptions[$r]))),
);
...
if ($isProviderAdmin) {
        ...
        $tenantWorkSections = [
            [
                ...
                'items' => [
                    ...
                    ['route' => 'tenant.tasks.index', 'icon' => 'fa-list-check', 'label' => 'Tasks', 'match' => 'tenant.tasks.*'],
                    ['route' => 'tenant.calendar.index', 'icon' => 'fa-calendar-days', 'label' => 'Calendar', 'match' => 'tenant.calendar.*'],
                    ['route' => 'tenant.time.index', 'icon' => 'fa-clock', 'label' => 'Time', 'match' => 'tenant.time.*'],
                ],
            ],
            [
                'label' => 'Sales',
                'items' => [
                    ['route' => 'tenant.leads.index', 'icon' => 'fa-user-plus', 'label' => 'Lead Manager', 'match' => 'tenant.leads.*'],
                    ['route' => 'tenant.opportunities.index', 'icon' => 'fa-chart-line', 'label' => 'Opportunities', 'match' => 'tenant.opportunities.*'],
                    ['route' => 'tenant.companies.index', 'icon' => 'fa-building', 'label' => 'Client Companies', 'match' => 'tenant.companies.*'],
                    ['route' => 'tenant.contacts.index', 'icon' => 'fa-user-group', 'label' => 'Clients', 'match' => 'tenant.contacts.*'],
                ],
            ],
            [
                'label' => 'Communication',
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'match' => 'tenant.settings.profile*',
                    ],
                    [
                        'route' => 'tenant.settings.api.index',
                        'icon' => 'fa-key',
                        'label' => 'API Keys',
                        'match' => 'tenant.settings.api.*',
                    ],
                    [
                        'route' => 'tenant.settings.billing',
                        'icon' => 'fa-wallet',
                        'label' => 'Subscription Billing',
                        'match' => 'tenant.settings.billing',
                    ],
                    [
                        'route' => 'tenant.settings.billing-upgrade',
                        'icon' => 'fa-arrow-up',
                        'label' => 'Upgrade',
                        'match' => 'tenant.settings.billing-upgrade',
                    ],
                ],
            ],
        ];
    ...
    $navSections = array_merge([['label' => 'Operations', 'items' => $operationsItems]], $tenantWorkSections);
    ...
    $navSections = [
        ...
        [
            'label' => 'Work',
            'items' => [
                ['route' => 'tenant.projects.index', 'icon' => 'fa-lightbulb', 'label' => 'Projects', 'match' => 'tenant.projects.*'],
                ['route' => 'tenant.tasks.index', 'icon' => 'fa-list-check', 'label' => 'Tasks', 'match' => 'tenant.tasks.*'],
                ['route' => 'tenant.calendar.index', 'icon' => 'fa-calendar-days', 'label' => 'Calendar', 'match' => 'tenant.calendar.*'],
                ['route' => 'tenant.time.index', 'icon' => 'fa-clock', 'label' => 'Time', 'match' => 'tenant.time.*'],
            ],
        ...
        [
            'label' => 'Settings',
            'items' => [
                ['route' => 'tenant.settings.index', 'icon' => 'fa-sliders', 'label' => 'Settings Overview', 'match' => 'tenant.settings.index'],
                ['route' => 'tenant.settings.profile', 'icon' => 'fa-palette', 'label' => 'Profile & Branding', 'match' => 'tenant.settings.profile*'],
                ['route' => 'tenant.settings.api.index', 'icon' => 'fa-key', 'label' => 'API Keys', 'match' => 'tenant.settings.api.*'],
                [
                    'route' => 'tenant.settings.billing',
                    'icon' => 'fa-wallet',
                    'label' => 'Subscription Billing',
                ...
                ['route' => 'tenant.settings.billing-upgrade', 'icon' => 'fa-arrow-up', 'label' => 'Upgrade', 'match' => 'tenant.settings.billing-upgrade'],
            ],
        ],
    ];
    ...
    if ($isTenantEmployee) {
        $navSections = array_values(array_filter(
            $navSections,
            fn($section) => ($section['label'] ?? '') !== 'Operations',
        ));
    }
...
@php
    ...
    $pinnedItems = array_values(array_filter(
        $pinnedItems,
        fn($item) => $resolveHref($item, $tenantParam) !== '#',
    ));
@endphp
        ...
        <div class="flex items-center justify-end">
            ...
            <nav class="sidebar-nav flex-1 space-y-3">
                ...
                @php
                    ...
                    $sectionKey = Str::of($section['label'] ?? 'section')->slug('-')->toString();
                @endphp
                <section class="oh-sidebar-section space-y-2" aria-expanded="{{ $isOpen ? 'true' : 'false' }}"
                    data-is-operations="{{ $isOperations ? '1' : '0' }}"
                    ...
                    <div id="{{ $sectionId }}" class="collapse-content sidebar-section-content flex flex-col gap-1">
