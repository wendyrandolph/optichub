...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        {{-- Header --}}
        <header class="mb-8">
            <h1 class="text-2xl font-semibold text-gray-900">Log Time Entry</h1>
            <p class="text-gray-600 mt-1">Record your work hours with an optional project or task.</p>
        </header>

        {{-- Flash Messages --}}
        @if (session('success_message'))
            <div
                class="mb-4 rounded-md bg-green-50 border border-green-200 text-green-800 p-3">
        ...
        @if ($errors->any())
            <div
                class="mb-4 rounded-md bg-red-50 border border-red-200 text-red-800 p-3">
        ...
        {{-- Form Card --}}
        <div
            class="rounded-xl bg-white shadow-card border border-gray-200 p-6">
            <form method="POST" action="{{ route('tenant.time.store', ['tenant' => request()->route('tenant')]) }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label for="hours" class="block text-sm font-medium text-gray-700">Total
                            Hours</label>
                        <input type="number" name="hours" id="hours" step="0.01" value="{{ old('hours') }}"
                            readonly required
                            class="mt-1 block w-full rounded-md bg-gray-100 border-gray-300">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="mt-1 block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500">{{ old('description') }}</textarea>
                </div>

                {{-- Submit --}}
                <div class="pt-4">
                    <button type="submit"
                        class="inline-flex items-center justify-center px-5 py-2 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition">
