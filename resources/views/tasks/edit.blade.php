...
@section('content')
    @php
        ...
        $assignId = $task->assign_id ?? null;

        $selectedTeamMemberId = $assignType === 'admin' ? old('assign_id', $assignId) : null;
        if (!$selectedTeamMemberId && !empty($task->user_id)) {
            $selectedTeamMemberId = collect($teamMembers ?? [])
                ->firstWhere('user_id', (int) $task->user_id)->id ?? null;
        }

        $selectedClientId = old('contact_id', $task->contact_id);
        if (!$selectedClientId && $assignType === 'client' && $assignId) {
            $selectedClientId = $assignId;
        }

        $currentAssignType = old('assign_type', $assignType ?: ($selectedClientId ? 'client' : 'admin'));
    @endphp

    <div class="oh-page">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="text-xs text-text-subtle">Tasks</div>
        ...
        <section class="oh-card p-4 sm:p-6 mt-4">
            <form method="POST" action="{{ route('tenant.tasks.update', ['tenant' => $tenantKey, 'task' => $task->id]) }}"
                    ...
                    <div class="oh-field">
                        ...
                        <select id="status" name="status" class="oh-select">
                            @foreach ($statusOptions as $value => $label)
                                <option value="{{ $value }}" @selected((string) $currentStatus === (string) $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <p class="oh-help text-status-danger">{{ $message }}</p>
                ...
                <div class="oh-form-grid">
                        ...
                        <div class="oh-field" data-assign-admin>
                            ...
                            <select id="assign_id" name="assign_id" class="oh-select">
                                ...
                                @foreach ($teamMembers ?? [] as $member)
                                    ...
                                    @endphp
                                    <option value="{{ $member->id }}" @selected((string) $selectedTeamMemberId === (string) $member->id)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="oh-field">
                        ...
                        <select id="assigned_user_id" name="assigned_user_id" class="oh-select">
                            ...
                            @foreach ($users ?? [] as $assignee)
                                ...
                                @endphp
                                <option value="{{ $assignee->id }}" @selected((string) old('assigned_user_id', $task->assigned_user_id ?? $task->user_id) === (string) $assignee->id)>
                                    {{ $assigneeName }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="oh-field">
                        ...
                        <input id="estimated_minutes" name="estimated_minutes" type="number" min="0" step="5"
                            value="{{ old('estimated_minutes', $task->estimated_minutes ?? '') }}" class="oh-input">
                ...
                <div class="oh-form-grid">
                    ...
                    <div class="oh-field">
                        ...
                        <input id="external_url" name="external_url" type="url"
                            value="{{ old('external_url', $task->external_url ?? '') }}" class="oh-input"
                            placeholder="https://..." />
                    ...
                    <div class="oh-field lg:col-span-2">
                        ...
                        <input id="feedback_image_url" name="feedback_image_url" type="url"
                            value="{{ old('feedback_image_url', $task->feedback_image_url ?? '') }}" class="oh-input"
                            placeholder="https://..." />
                ...
                <div class="grid gap-3 lg:grid-cols-2">
                    <label class="flex items-start gap-3 rounded-xl p-3 border border-[rgb(var(--border))] bg-surface-card">
                        <input type="checkbox" name="client_must_upload" value="1" class="mt-1"
                            @checked(old('client_must_upload', (bool) ($task->client_must_upload ?? false))) />
                        <div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (!assignType || !memberSelect || !clientSelect || !clientAssignId || !adminField || !clientField) return;
