...
class TeamMemberController extends Controller
{
    public function index(Tenant $tenant): View
    {
        ...
        // If you truly still need to ensure "admin" users have a TeamMember record,
        // keep it - but do it WITHOUT early returns.
        ...
        // Choose wrapper view based on workspace
        $workspaceType = strtolower((string) ($tenant->workspace_type ?? 'creative'));
        $view = $workspaceType === 'trades'
            ? 'trades.team-members.index'
            : 'team-members.index';

        return view($view, compact('tenant', 'members', 'search', 'roleFilter', 'statusFilter', 'sort'));
    }
    /** Show create form */
    public function create(Tenant $tenant): View
    {
        ...
        $workspaceType = strtolower((string) ($tenant->workspace_type ?? 'creative'));
        $view = $workspaceType === 'trades'
            ? 'trades.team-members.create'
            : 'team-members.create';

        $roles = $workspaceType === 'trades'
            ? ['admin', 'dispatcher', 'lead_tech', 'tech']
            : ['admin', 'member', 'viewer'];

        $defaultRole = $workspaceType === 'trades' ? 'tech' : 'member';
        ...
        return view($view, compact('tenant', 'roles', 'defaultRole', 'usedColors'));
    }
    ...
    public function store(Request $request, Tenant $tenant): RedirectResponse
    {
        ...
        $user = \App\Models\User::create([
            ...
            'password' => $password ? \Illuminate\Support\Facades\Hash::make($password) : \Illuminate\Support\Str::random(12),
            'tenant_id' => $tenant->id,
            'role' => $validated['role'],
            'must_change_password' => $password ? false : true,
            'hired_at' => $isTrades && Schema::hasColumn('users', 'hired_at') ? ($validated['hired_at'] ?? null) : null,
        ...
        return redirect()->route('tenant.team-members.index', ['tenant' => $tenant->id])
    ...
    public function show(Tenant $tenant, TeamMember $team_member): View
    {
        ...
        // Determine last login from related user login activities (fallback null)
        $lastLogin = null;
        if ($memberUserId) {
            $lastLogin = User::find($memberUserId)?->loginActivities()->max('created_at');
        }

        // Open projects (anything not completed/cancelled/closed) where this member is owner/PM/user
        ...
        $openProjects = $projectsQuery->get();

        // Open tasks (todo/in_progress) assigned to this member or their linked user
        ...
        $openTasks = $openTasksQuery->get();

        // Opportunities owned
        $ownedOppsQuery = Opportunity::query()
            ->where('tenant_id', $tenant->id);
        if ($memberUserId) {
            $ownedOppsQuery->where('owner_id', $memberUserId);
        } else {
            $ownedOppsQuery->whereRaw('1 = 0'); // none if no linked user
        }
    ...
    public function update(Request $request, Tenant $tenant, TeamMember $team_member): RedirectResponse
    {
        ...
        if ($user && $isTrades && Schema::hasColumn('users', 'hired_at') && array_key_exists('hired_at', $validated)) {
